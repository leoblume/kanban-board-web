...
// NOVO: Coleção para a programação semanal
...
// --- LISTENER PRINCIPAL DO BANCO DE DADOS (KANBAN) ---
...
onSnapshot(q, (snapshot) => {
    ...
    if (updatesNeeded > 0) { batch.commit().catch(err => console.error("Erro ao salvar auto-correção de dados:", err)); }
...
// --- NOVO: LÓGICA DA PROGRAMAÇÃO SEMANAL ---

// Função para renderizar um único item na programação
function renderScheduleItem(os, client) {
    ...
    item.innerHTML = `
        <span class="schedule-item-text">${os} / ${client}</span>
        <button class="delete-schedule-item-btn" title="Excluir">×</button>
    `;
    return item;
}

// Listener para carregar e manter a programação sincronizada com o Firebase
onSnapshot(scheduleCollection, (snapshot) => {
    ...
    snapshot.forEach(doc => {
        ...
        if (zone) {
            tasks.forEach(task => {
                zone.appendChild(renderScheduleItem(task.osNumber, task.clientName));
            });
        }
    });
});

// Listeners para drag-and-drop na programação
    ...
    if (dropZone) {
        ...
        try {
            ...
            if (taskData.osNumber && taskData.clientName) {
                ...
                // Usa setDoc com merge para criar o doc se não existir, e arrayUnion para adicionar a tarefa
...
// Listener para excluir item da programação
weeklySchedulePanel.addEventListener('click', async e => {
    if (e.target.classList.contains('delete-schedule-item-btn')) {
        ...
        if (item && zone) {
            ...
            const scheduleDocRef = doc(db, "schedule", dayId);
            try {
                // Usa updateDoc com arrayRemove para remover a tarefa do array
